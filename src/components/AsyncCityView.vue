<template>
    <div class="flex flex-col flex-1 items-center">
        ...
        <div v-if="route.query.preview"
            class="w-full text-white bg-weather-secondary py-2 text-center">
            <p>You are currently previewing the city, click the "+" icon to start tracking this city</p>
        </div>
        <!--Weather Overview-->
        <div class="flex flex-col items-center text-white py-12">
            <h1 class="text-4xl mb-2">{{ route.params.city}}</h1>
            <h2 class="text-xl mb-2">{{route.params.state}}</h2>
            ...
            <p class="text-8xl mb-8">
                {{ weatherCel(weatherData.current.temp) }}&deg;
            </p>
            
            <p>Feels like: {{ weatherCel(weatherData.current.feels_like)}}</p>
            ...
            <div class="mx-8 text-white">
                ...
                <div class="flex gap-10 overflow-x-scroll">
                    <div 
                        ...
                        >
                        <p class="text-sm"> {{hourData.weather[0].main}} </p>
                        <p> {{ weatherCel(hourData.temp)}}&deg; </p>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="mx-8 text-white">
                <h2 class="mb-4">Weekly Weather</h2>
                    <div 
                        ...
                        :key="day.dt"
                    >
                        <p class="flex-1"> {{ dayLong(day.dt) }} </p>
                        <img 
                        ...
                        <div class="flex gap-2 flex-1 justify-end">
                            <p>H: {{weatherCel(day.temp.max)}}&deg;</p>
                            <p>L: {{weatherCel(day.temp.min)}}&deg;</p>
                        </div>
                    </div>
            </div>
        </div>
...
<script setup>
    ...
    const dayLong = (x)=>{
        const y = new Date(x*1000).toLocaleDateString("en-us",{weekday:"long",})
        return y
    }
